...
object Passes {
  ...
  val OO: Seq[AbstractPass] = Seq(
    ...
    SimplePass(
      "monomorphizeGenericClass", "Monomorphize generic class for all its instances",
      new MonomorphizeGenericClass(_).rewriteAll(),
      permits=Set(features.AbstractClass),
      removes=Set(features.AbstractClass),
    ),
    SimplePass(
      "monomorphizeGenericFunctions", "Monomorphize generic function for all its instances",
      new MonomorphizeGenericFunctions(_).rewriteAll(),
      permits=Feature.ALL,
      removes=Set(features.AbstractMethod),
    ),
  )
